...
pub async fn publish_newsletter(
    ...
    // Retrieve the credentials from the header
    ...
    let user_id = validate_credentials(credentials, &pool)
        .await
        // We match on `AuthError`'s variants, but we pass the
        // **whole** error into the constructors for `PublishError`
        // variants. This ensures that the context of the top-level
        // wrapper is preserved when the error is
    ...
    // Now get the confirmed subscribers
